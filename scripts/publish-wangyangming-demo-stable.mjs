...
const created = await api('POST', `/api/scenarios/${scenarioId}/versions`, {
  ...
  // 演示稳定版：不强制 structuredOutput，避免模型偶发漏字段退回原文摘录
  outputSchema: { type: 'object', properties: {}, required: [] },
  evaluationCases: [
    {
      question: '结合《王阳明传奇》转写，龙场悟道的核心体会是什么？',
      expectedTerms: ['龙场'],
      mustRefuse: false,
      baseId,
    },
...
console.log(JSON.stringify(evaluation?.results?.map(r => ({
  i: r.index, passed: r.passed, mode: r.mode, missing: r.missingTerms, warning: r.warning,
})), null, 2));
...
if (evaluation?.passed === evaluation?.total && evaluation?.total > 0) {
  ...
  console.log('PUBLISHED', pub.version.status, pub.version.id, 'v' + pub.version.version);
